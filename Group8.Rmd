...
library(foreign)
library(gplots)
plotmeans(sales~ year, main="Heterogeineity across year", data=Cigar,n.label = FALSE)
...
plotmeans(sales~ state, main="Heterogeineity across state", data=Cigar,n.label = FALSE)
...
We fit a fixed effects model with *sale* as the response, *price_adj*, *pop*, *pop16* and *income_adj* as independent variables, and *state* and *year* as fixed effects variables.
...
# Use plm package for *state* and *year* and fit a model
library(plm)

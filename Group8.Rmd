...
There are three ways to do with R, using the regular commands **lm**, the **felm** command in the lfe package, or **plm** in the plm package. In fact, they produce the same results. 

The **lm** generates dummies variables for *state* and *year* and then run linear regression. And the **felm** and **plm** will absorb the estimates of specific groups. 

If we just want to control for fix effect and only care about another coefficients of interests, either **felm** and **plm** is a good choice. But if if we want to know the effect of some specific groups, **lm** is preferred. 
...
It we only want to know the estimates of our interests, we can use 'absorb' instead of 'class'. But it can only absorb one variable at a time.
